package com.hiwayne.commons.file;
...
/**
 * 
 * @Description: 这个类提供了一些根据类的class文件位置来定位的方法。用户获取相对路径
 * @date 2012
 * @version V1.0
 * @author wangwei
 * @since wangwei
...
public class Path {
	 ...
	 * @throws IOException
	 */
	public static String getWeb_FullPathRelateClass(String relatedPath)
			throws IOException {
		String path = null;
		if (relatedPath == null) {
			throw new NullPointerException();
		}
		String clsPath = getPathFromClass(new Path().getClass());
		File clsFile = new File(clsPath);
