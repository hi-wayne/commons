...
/**
 * 先拿memcache内信息，如果没有就进行保存操作
 * 
 * @author wangwei
 * 
 */
@Aspect
public class CHashingCacheGetAspectJ {
	private static final Logger logger = LoggerFactory
			.getLogger(CHashingCacheGetAspectJ.class);
	...
	@Pointcut("@annotation(com.hiwayne.commons.aspect.cache.CHashingCacheGetAnnotation)")
	...
	public Object methodCacheHold(ProceedingJoinPoint joinPoint)
		...
		for (Method m : method) {
			if (m.getName().equals(methodName)) {
				if (m.getParameterTypes().length == arguments.length
					...
					CHashingCacheGetAnnotation aCHashingCacheGetAnnotation = m
							.getAnnotation(CHashingCacheGetAnnotation.class);
					keyPrefix = aCHashingCacheGetAnnotation.keyPrefix();
					maxLeftSecond = aCHashingCacheGetAnnotation.maxLeftSecond();
					if (maxLeftSecond <= 0) {
						maxLeftSecond = 0;// 0为永远不超时
					}
					mutual = aCHashingCacheGetAnnotation.mutual();
					useCacheIndex = aCHashingCacheGetAnnotation.useCacheIndex();
					leftSecondIndex = aCHashingCacheGetAnnotation
							.leftSecondIndex();
					keyParMainIndex = aCHashingCacheGetAnnotation
							.keyParMainIndex();
					keyParSubIndex = aCHashingCacheGetAnnotation
							.keyParSubIndex();
					keyParSub2Index = aCHashingCacheGetAnnotation
							.keyParSub2Index();
					keyParSub3Index = aCHashingCacheGetAnnotation
							.keyParSub3Index();
					saveNull = aCHashingCacheGetAnnotation.saveNull();
					saveNullLeftSecond = aCHashingCacheGetAnnotation
							.saveNullLeftSecond();
					cacheKeyMD5 = aCHashingCacheGetAnnotation.cacheKeyMD5();
					getMethodInfo = true;
					break;
				}
			}
		...
		// 查询cache(并且验证cache的有效性)
		boolean invoke = true;
		if (useCache) {
			resultObj = memcachedClient.get(cacheKey);
		}
		if (resultObj == null) {
			invoke = true;
			if (useCache) {
				if (logger.isInfoEnabled()) {
					runInfo.append(",memcache内没有信息");
				}
			} else {
				if (logger.isInfoEnabled()) {
					runInfo.append(",不要求查询memcache");
				}
			}
		} else {
			if (resultObj instanceof VersionData) {// 有版本信息的数据
				boolean isExist = ((VersionData) resultObj).isExist();
				boolean isBeforehandLost = ((VersionData) resultObj)
						.isBeforehandLost();
				if (isExist) {
					if (isBeforehandLost) {
						invoke = true;
						((VersionData) resultObj).delayLoseThis();
						memcachedClient.set(cacheKey, maxLeftSecond, resultObj);
						if (logger.isInfoEnabled()) {
							runInfo.append(",预互斥处理-cache已过期 ");
						}
					} else {
						invoke = false;
						if (logger.isInfoEnabled()) {
							runInfo.append(",预互斥处理-cache还未过期 =");
						}
					}
				} else {
					invoke = true;
					if (logger.isInfoEnabled()) {
						runInfo.append(",预互斥处理-查询信息不存在");
					}
				}
				...
				invoke = false;
				if (logger.isInfoEnabled()) {
					runInfo.append(",成功获取memcache内无版本信息");
				}
			}
		}
		...
		if (invoke) {
			if (mutual) {
				...
				if (!lockFlag) {
					for (int i = 0; i < CASCOUNT; i++) {
						...
						lockFlag = memcachedClient
								.add(mutualKey, MUTUALTIME, 1);
						if (lockFlag) {
							break;
						} else {
							if (logger.isInfoEnabled()) {
			...
			if (useCache) {
				if (resultObj != null) {
					if (resultObj instanceof VersionData) {// 有版本信息的数据
						...
						if (!addFlag) {// cache中有数据add失败
							...
							if (aGetsResponse != null) {// 处理cache突然挂掉
								...
								if (cacheDate instanceof VersionData) {
									...
									if (cacheVersionData != null
										...
										// 如果按cas值进行更新失败，表示在更新过程中cache值又发生后了变化,此操作默认失败
										boolean upFlag = memcachedClient.cas(
												cacheKey, maxLeftSecond,
												resultObj, casVersion);// 按cas值进行更新
										if (!upFlag) { 
											if (logger.isInfoEnabled()) {
												runInfo.append(",用原生方法结果写cache cas失败");
											}
										} else {
											if (logger.isInfoEnabled()) {
												runInfo.append(",用原生方法结果写cache cas成功");
											}
										}
						...
						if (isSaveNullObject) {
							...
							memcachedClient.set(cacheKey, maxLeftSecond,
									resultObj);
						}
				...
				if (logger.isInfoEnabled()) {
					runInfo.append(",使用原生方法结果,不要求进行回写cache操作");
				}
			}
		} else {
			if (logger.isInfoEnabled()) {
				runInfo.append(",直接从cache拿结果");
			}
		}
